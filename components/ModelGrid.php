...
function render_model_grid_with_filters()
{
  ...
  // --- 5. Определение LCP (Приоритетная загрузка) ---
  $is_mobile = wp_is_mobile();

  // Логика:
  if (is_front_page() || is_home()) {
    // На ГЛАВНОЙ все в сетке грузим лениво (так как выше есть слайдер)
    $lcp_limit = 0;
  } else {
    // На ОСТАЛЬНЫХ: ПК = 4, Моб = 1
    $lcp_limit = $is_mobile ? 1 : 4;
  }

  /* ===== Таксономии ===== */
  $ALLOWED_TAX = ['ves_tax', 'vozrast_tax', 'grud_tax', 'drygie_tax', 'metro_tax', 'nationalnost_tax', 'rayonu_tax', 'rost_tax', 'cvet-volos_tax', 'price_tax', 'vneshnost_tax', 'figura_tax', 'uslugi_tax'];
  $base_tax = [];
  ...
  if (!$is_novye) {
    ...
    if (empty($base_tax)) {
      ...
      // Посадочные CPT, где slug страницы совпадает с термином нужной таксы
      $CPT_TAX_MAP = [
        'tsena'         => 'price_tax',
        'vozrast'       => 'vozrast_tax',
  ...
  // Страница "Кыздар нет": жёстко фильтруем по национальности "kazashki"
  if ($is_kizdar_page) {
  ...
  if (!$is_novye) {
    foreach ($ALLOWED_TAX as $tx) {
     ...
     if (!empty($vals)) $tax_query[] = ['taxonomy' => $tx, 'field' => 'slug', 'terms' => $vals, 'operator' => 'IN'];
    }
    if (!empty($base_tax)) array_unshift($tax_query, ['taxonomy' => $base_tax['taxonomy'], 'field' => 'term_id', 'terms' => $base_tax['terms'], 'operator' => 'IN']);
  ...
  $use_offset_mode = !empty($_POST['use_offset']) && $_POST['use_offset'] == '1';
  ...
  if ($is_novye) {
    ...
    if (empty($candidates)) $args['post__in'] = [0];
    else $args['post__in'] = array_slice($candidates, 0, $per_page + 1);
    $args['paged'] = 1;
    unset($args['post__not_in']);
} else {
    // 1. Если это ПОДГРУЗКА (страница 2, 3... или нажата кнопка "Показать еще")
    if ($append || $paged > 1) {
        // Исключаем ВСЁ, что уже есть на экране, чтобы не было дублей при скролле
        if (!empty($all_ex)) {
            $args['post__not_in'] = $all_ex;
        }
    } 
    else {
        if (!empty($exclude_ids_static)) {
            $args['post__not_in'] = $exclude_ids_static;
        }
    }
}

  if ($cheap_only) {
    if (!isset($args['tax_query'])) $args['tax_query'] = ['relation' => 'AND'];
    elseif (!empty($tax_query)) $args['tax_query'] = $tax_query;
    // Не требуем price_tax, отбираем по цене через meta_query ниже
  } elseif ($elite_only) {
    if (!isset($args['tax_query'])) $args['tax_query'] = ['relation' => 'AND'];
    elseif (!empty($tax_query)) $args['tax_query'] = $tax_query;
    // Не требуем price_tax, отбираем по цене через meta_query ниже
  } elseif (!empty($tax_query) && !$is_novye) {
  ...
  $distinct_cb = null;
  $groupby_cb = null;
  if (!empty($args['tax_query']) || !empty($tax_query)) {
  ...
  if (!empty($found_ids)) {
    ...
    foreach ($raw_meta as $row) {
      ...
      elseif (isset($v['ID'])) $att_ids[] = $v['ID'];
    }
  ...
  if (!$append) {
    ...
    $h2_before_grid = $is_novye ? 'Новые анкеты' : '';
  }
    ...
    foreach ($found_ids as $index => $pid) {
      ...
      } elseif (is_numeric($raw_gallery)) $gallery = [(int)$raw_gallery];
      if (empty($gallery)) continue;

      // === ЛОГИКА ПРИОРИТЕТА ===
      // Если индекс меньше лимита (0 для главной, 1 для моб, 4 для пк) - грузим быстро
...
<?php endif;
    ...
    if (!$append && $paged === 1 && empty($exclude_ids_rendered)) echo '<p class="text-neutral-600">Модели не найдены.</p>';
  endif;
